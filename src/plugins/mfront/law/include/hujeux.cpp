...
namespace hujeux_law {

    using namespace std;

    template<typename T>
    void gauss(std::vector <T> *a, std::vector <T> &b, std::vector <T> &x, const int &n);

    const auto EPS = 1.e-15;
    const auto FTOL = 1.e-8;
    const auto Pa = -1.e6;                      // Reference stress (in Pa)
    const auto RAD = acos(-1.) / 180.;
    string ferrlog{"error.log"};
    char szout[MAXBUFFER];

/*---------------------------------------------------------------------------*/
    const int ipermu[3][3] = {{1, 2, 5},
                              {2, 0, 4},
                              {0, 1, 3}};
    constexpr auto NHISTHUJ = 24; // nb of internal variables (history) for Hujeux
    constexpr auto NPROPHUJ = 24; // nb of const properties (user data) for Hujeux
...
//=================================================================================================================//
//=================================================================================================================//
    Hujeux3D::Hujeux3D(const Hujeux3D &h) {
        init(h.m_param);
    }
//=================================================================================================================//
/**/
...
//=================================================================================================================//
    void Hujeux3D::init(const dvector &param) {
...
//=================================================================================================================//	        
    bool Hujeux3D::readParameters(const string &name) {
#ifdef DEBUG
        cout << "\n Entering class Hujeux3D::readParameters  from hujeux.cpp\n"
           << " Input parameters :\n                  name = "  << name <<"\n"<< endl;
#endif
        ...
        if (name.empty())
        {
            cout << "\n Error filename for materials not found "
                    "--> Process stopped in Hujeux3D::readParameters\n";
            return false;
        }
#endif
...
//=================================================================================================================//
//=================================================================================================================//
    void Hujeux3D::set_ipl(const int &iiplval) {
        iipl = iiplval;
        ipl[3] = iipl / 125;
...
//=================================================================================================================//
    void Hujeux3D::computeElastTensor(const Real &p) {
        auto K = m_param[0];
        m_mu = m_param[1];
        auto ne = m_param[2];
        Real fac = 1.;
...
//=================================================================================================================//
    void Hujeux3D::computeTangentTensor(const Tensor2 &sig) {
        auto p = trace(sig) / 3.;
...
//=================================================================================================================//
//=================================================================================================================//
    void Hujeux3D::initHistory(dvector *histab) {
        evp = histab->front();
...
// 
//=================================================================================================================//
    bool Hujeux3D::initState(const Tensor2 &sig, dvector *histab) {
...
//=================================================================================================================//
//=================================================================================================================//
    bool Hujeux3D::initMecdev(const Tensor2 &sig, int &iniplk, int &iplk, Real2 &vnk, Real2 &sbk, Real2 &rayk) {
        ...
        if (iniplk == 1) {
            if (raykk > 1.) {
                ...
                ficherr.open(ferrlog.c_str(), ios::out | ios::app);
                ostream oserr(&ficherr);
                snprintf(szout, MAXBUFFER, "Starting out of dev yield surface %d!",
                         km + 1);             ///////// CHECK CHECK
                oserr << szout << endl;
                ficherr.close();
                raykk = 1.;
...
//=================================================================================================================//
//=================================================================================================================//
    bool Hujeux3D::initMeciso(const Tensor2 &sig, int &inipl4, int &ipl4, Real2 &delta) {
...
//=================================================================================================================//
    void
    Hujeux3D::ComputeMecdev(const Tensor2 &sig, const Tensor2 &dsig, Real *Phik, Real *Psik, Real *CPsik, Real &seuilk,
                            Real &fidsig, Real &hray, Real &xlray, int &iplk, int &jplk, Real2 &vnk, Real2 &sbk,
                            Real2 &rayk) {
        Real2 c, cred, s, si;
        auto pci = m_param[6];
        auto b = m_param[8];
        ...
        fidsig = Phik[im] * dsig.m_vec[im] + Phik[jm] * dsig.m_vec[jm] + Phik[km] * dsig.m_vec[km] +
                 Phik[4] * dsig.m_vec[lm];
...
//	if (fidsig*inc != 0. && fabs(1. + seuilk/(fidsig*inc)) < EPS) actif = 0.;
        ...
        if (actif > 0.) {
            ...
            switch (jecoul) {
                default:
                case 1:
                    psiv = (sinpsi - (dfds[0] * si[0] + dfds[1] * si[1]) / pk);
                    break;
...
//=================================================================================================================//
    void
    Hujeux3D::ComputeMeciso(const Tensor2 &sig, const Tensor2 &dsig, Real *Phic, Real *Psic, Real *CPsic, Real &seuilc,
                            Real &fidsig,
...
//=================================================================================================================//
    void Hujeux3D::ComputeStress(dvector *histab, Tensor2 &sig, Tensor2 &eps, Tensor2 &epsp, Tensor2 &dsig,
        ...
        beta = m_param[5],
                ...
                daux[6][6];
        darray4 seuil{0.}, fidsig{0.}, hray{0.}, xlray{0.}, hb{0.};
        auto hh1 = new Real *[4];
//    auto hb = new Real[4];
...
//=================================================================================================================//
    int Hujeux3D::TestClass() {

#ifdef DEBUG
        cout << "\n Entering class Hujeux3D::getNum  from hujeux.cpp "<<endl;
#endif
