...
if(dirichletpointconditions<1 && pointprobe && Model!="pseudo_nonlinear"){
 ...
 "  real err1Gather, duNormGather, uNormGather;                                   \n"
 "  real duNormLoc, uNormLoc;                                                     \n"
 "  real[int] normWork(Vh.ndof);                                                  \n"
 "                                                                                \n"
 "  // Relative Newton-increment norm:                                            \n"
 "  // err = ||Delta u||_2 / (1 + ||u||_2), with Delta u represented by du.      \n"
 "  // DP removes duplicate interface-DOF contributions in the MPI sum.          \n"
 "  normWork = du[] .* du[];                                                      \n"
 "  normWork = normWork .* DP;                                                    \n"
 "  duNormLoc = normWork.sum;                                                     \n"
 "  normWork = u[] .* u[];                                                        \n"
 "  normWork = normWork .* DP;                                                    \n"
 "  uNormLoc = normWork.sum;                                                      \n"
 "  mpiAllReduce(duNormLoc,duNormGather,mpiCommWorld,mpiSUM);                      \n"
 "  mpiAllReduce(uNormLoc,uNormGather,mpiCommWorld,mpiSUM);                        \n"
 "  err1Gather = sqrt(duNormGather)/(1. + sqrt(uNormGather));                      \n"
 "                                                                                \n"
 ...
 "      << \" ]\\nRelative L2 Newton increment :  [ \" << err1Gather                   \n"
 "      << \" ]\"                                                                 \n"
 "      << \"\\n----------------------------------------\\n\"                     \n"
 "      << endl;                                                                  \n"
 "                                                                                \n"
