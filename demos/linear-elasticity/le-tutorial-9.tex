...
via \psd{PsdMfrontHandler(...)} function
...
The two raised points are handled using \psd{PsdMfrontHandler(...)} in
\psd{FemParameters.edp} file.
...
Finally in file \psd{FemParameters.edp} the \psd{PsdMfrontHandler()} is
called to build the material tensor \psd{Mt} provided with the
