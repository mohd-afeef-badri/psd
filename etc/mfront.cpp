...
AnyType MFrontParameter_Op<K>::operator()(Stack stack) const {
    double myMu  = GetAny<double>((*mu)(stack))		;
    constexpr const auto yg = mgis::real{150e9};
    constexpr const auto h = Hypothesis::TRIDIMENSIONAL;
    const auto d = load("/home/mb258512/Work/repo/psd_sources/src/plugins/mfront/law/elasticity/src/libBehaviour.so", "Elasticity", h);
    cout
             << " PSD MFront plugin prints "
             <<  yg //<< "  " << d.behaviour
             <<
    endl;
